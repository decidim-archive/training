...
const compileCurriculas = curriculas =>
  _.reduce((memo, [, zipper]) => {
    ...
    return _.concat(memo, {title, id, item, duration, meta, tags,
                           materials: _.map(m =>
                             _.merge({}, {id: m})
...
const fetchJSON = (locale, filename) =>
  fs.readFileSync(`${__dirname}/../../content/${locale}/${filename}.json`, 'utf8');
...
export const plugin = () =>
  (files, metalsmith, done) => {
    ...
    const allData = locale => {
      const curriculas = _.filter({meta: {locale}},
      _.concat(compileCurriculas(transformed.curriculas),
      compileCurriculas(transformed.activities)));
      const filters = _.uniq(_.flatMap('tags', curriculas));
      const d = _.merge({}, {
        curriculas,
        activities: _.filter({meta: {locale}}, compileCurriculas(transformed.activities)),
