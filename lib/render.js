...
const compactObj = obj => _.reduce((memo, [key, value]) => (_.isNil(value)
    ? memo
    : _.merge(memo, {[key]: value})), {}, _.toPairs(obj));

const mergeConcatArrays = (objValue, srcValue) => (_.isArray(objValue)
    ? _.concat(objValue, srcValue)
    : undefined);
...
const compileCurriculas = curriculas => _.reduce((memo, [, zipper]) => {
  const title = sectionTitle(zipper);
  ...
  const [prerequesites,
      sections] = _.map(subSections, subSections(zipper));
  ...
  return _.concat(memo, {
    ...
    meta,
    tags,
    materials: _.map(m => _.merge({}, {id: m}), _.uniq(zipper.meta.materials || [])),
    activities: _.map(a => _.merge({}, {id: a}), _.uniq(zipper.meta.activities || [])),
...
const markerTransform = _.curry((fieldName, metaData, zipper) => {
  ...
  zipper.meta = _.mergeWith(mergeConcatArrays, zipper.meta, {[fieldName]: [uuid]});
  return zipper;
});

const fetchJSON = (locale, filename) => fs.readFileSync(`${__dirname}/../config/${locale}/${filename}.json`, 'utf8');

export const plugin = () => (files, metalsmith, done) => {
  ...
  const data = _.reduce((memo, [collection, collectionFiles]) => _.merge(memo, {
    [collection]: _.reduce((acc, file) => _.merge(acc, {
      [file.path.href]: file.transmark
    }), {}, collectionFiles)
  }), defaultData, orderedColl);

  const transformed = _.reduce((memo, [collection, collectionFiles]) => _.merge(memo, {
    [collection]:
    _.reduce((acc, [path, file]) =>
      _.merge(acc, {[path]: transformer(file)}), {}, _.toPairs(collectionFiles))
  }), {}, _.toPairs(data));
  ...
  const allData = locale => {
    ...
    }, _.concat(compileCurriculas(transformed.curriculas), _.concat(methodologies, activities)));
    const filters = _.uniq(_.flatMap('tags', curriculas));
    const types = _.uniq(_.flatMap('item', curriculas));
    const d = _.merge({}, {
      curriculas,
      activities,
      methodologies,
      materials: _.filter({
        locale
      }, compileMaterials(transmark.tags.material)),
      filters,
      types
    });
    return d;
  ...
  _.forEach(locale => {
    ...
    files[`${locale.code}-data.json`] = {
      contents: new Buffer(d)
    };

    winston.info(`Writing UI strings for ${locale.name} - (${locale.code})`);
    files[`${locale.code}-ui.json`] = {
      contents: new Buffer(ui)
    };
  }, locales);
